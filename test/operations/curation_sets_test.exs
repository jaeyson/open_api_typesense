defmodule OpenApiTypesense.CurationSetsTest do
  ...
  setup_all do
    ...
    on_exit(fn ->
      curation_sets =
        case CurationSets.retrieve_curation_sets() do
          {:ok, sets} ->
            sets
      ...
      if Enum.any?(curation_sets) do
        Enum.each(curation_sets, fn set ->
          {:ok, _set} = CurationSets.delete_curation_set(set.name)
        end)
      end
    end)
  ...
  test "success: retrieve a curation set", %{conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, %CurationSetSchema{name: ^name}} =
             CurationSets.upsert_curation_set(name, curation_set)

    assert {:ok, %CurationSetSchema{name: ^name}} = CurationSets.retrieve_curation_set(name)
    assert {:ok, %CurationSetSchema{name: ^name}} = CurationSets.retrieve_curation_set(name, [])

    assert {:ok, %CurationSetSchema{name: ^name}} =
             CurationSets.retrieve_curation_set(name, conn: conn)
  ...
  test "error: retrieve a non-existing curation set", %{conn: conn, map_conn: map_conn} do
    ...
    assert ^error = CurationSets.retrieve_curation_set(set_name)
    assert ^error = CurationSets.retrieve_curation_set(set_name, [])
    assert ^error = CurationSets.retrieve_curation_set(set_name, conn: conn)
  ...
  test "error: retrieve a non-existing curation set item", %{conn: conn, map_conn: map_conn} do
    ...
    assert ^error = CurationSets.retrieve_curation_set_item(name, item_id)
    assert ^error = CurationSets.retrieve_curation_set_item(name, item_id, [])
    assert ^error = CurationSets.retrieve_curation_set_item(name, item_id, conn: conn)
  ...
  test "error: retrieving an item_id with a whitespace on its name", %{
    ...
    assert ^error = CurationSets.retrieve_curation_set_item(set_name, item_id)
    assert ^error = CurationSets.retrieve_curation_set_item(set_name, item_id, [])
    assert ^error = CurationSets.retrieve_curation_set_item(set_name, item_id, conn: conn)
  ...
  test "success: list all curation sets", %{conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, curation_sets} = CurationSets.retrieve_curation_sets()
    assert Enum.any?(curation_sets)
    assert {:ok, _} = CurationSets.retrieve_curation_sets([])
    assert {:ok, _} = CurationSets.retrieve_curation_sets(conn: conn)
  ...
  test "success: list all curation set items", %{conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, %CurationSetSchema{name: ^set_name}} =
             CurationSets.upsert_curation_set(set_name, curation_set)
    ...
    assert {:ok, %CurationItemSchema{id: ^item_id}} =
             CurationSets.upsert_curation_set_item(set_name, item_id, body)

    assert {:ok, set_items} = CurationSets.retrieve_curation_set_items(set_name)
    assert Enum.any?(set_items)
    assert {:ok, _set_items} = CurationSets.retrieve_curation_set_items(set_name, [])
    assert {:ok, _set_items} = CurationSets.retrieve_curation_set_items(set_name, conn: conn)
  ...
  test "success: create or update a curation set", %{conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, %CurationSetSchema{name: ^name}} =
             CurationSets.upsert_curation_set(name, curation_set, conn: conn)
  ...
  test "success: upsert a curation set item", %{conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, %CurationItemSchema{id: ^item_id}} =
             CurationSets.upsert_curation_set_item(set_name, item_id, body, conn: conn)
  ...
  test "success: delete a curation set", %{conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, %CurationSetDeleteSchema{name: ^name}} = CurationSets.delete_curation_set(name)

    error = {:error, %ApiResponse{message: "Curation index not found"}}
    assert ^error = CurationSets.delete_curation_set(name, [])
    assert ^error = CurationSets.delete_curation_set(name, conn: conn)
  ...
  test "success: delete a curation set item", %{conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, %CurationItemSchema{id: ^item_id}} =
             CurationSets.upsert_curation_set_item(name, item_id, body)

    assert {:ok, %CurationItemDeleteSchema{id: ^item_id}} =
             CurationSets.delete_curation_set_item(name, item_id)

    error = {:error, %ApiResponse{message: "Could not find that `id`."}}

    assert ^error = CurationSets.delete_curation_set_item(name, item_id, [])
    assert ^error = CurationSets.delete_curation_set_item(name, item_id, conn: conn)
    assert ^error = CurationSets.delete_curation_set_item(name, item_id, map_conn: map_conn)
  end
end
